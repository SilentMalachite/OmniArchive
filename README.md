...
- **セキュリティと入力制限**: XSS やリソース枯渇攻撃を防ぐため、メタデータ入力（キャプション、遺跡名等）に厳格な文字数制限を導入。

### ⚡ 並列処理パイプライン & バックグラウンド処理
...
- **DEFLATE 可逆圧縮**: PTIFF 生成に DEFLATE 圧縮を採用。考古学資料の線画に発生しがちなモスキートノイズを完全に防止
...
| フロントエンド | Phoenix LiveView + 独自の JavaScript Hook (ImageSelection ポリゴン描画) |
...
| モード | 説明 |
|:---:|:---|
| 🖤 **モノクロモード（高速）** | `-gray` フラグ付きでグレースケール変換。線画中心の報告書に最適（デフォルト） |
| 🎨 **カラーモード（標準）** | フルカラーで出力。写真や彩色図を含む報告書向け |
