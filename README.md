...
| **4. ラベリング** | 🏷️ | メタデータを入力します。さらに、任意のキーと値のペアを追加できる**動的カスタムフィールド**も利用可能です。保存完了後、自動的に PTIF 生成がバックグラウンドで開始されます |
...
- **ファセット検索**: 動的に追加されたカスタムフィールドキー・値によるフィルタリング
...
- **厳格なバリデーション**: ラベル形式 (`fig-1-1` など) の強制により、識別子の一貫性を担保
...
キャプション（図の説明）、ラベル（識別名）、および任意のキー・値を持つ動的メタデータを入力してください。
...
`/lab/search` で、キャプションや動的に追加した任意のメタデータキー・値から画像を検索できます。
