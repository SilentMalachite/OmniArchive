...
### 🏗️ 汎用的ドメインモデルへの移行
- **考古学固有フィールドの削除と動的メタデータへの統合**
  - `ExtractedImage` および関連する `Item` スキーマから `site` (遺跡名)、`period` (時代)、`artifact_type` (遺物種別) などの考古学ドメインに依存するカラムを完全に削除。
  - これらの固定フィールドを、前バージョン(0.2.12)で導入した `custom_metadata` (JSONB) へ統合し、OmniArchive を任意のドメイン(古文書、設計図、博物資料など)で利用可能な汎用データ基盤へリファクタリング。
  - 特定のフィールドに依存したバリデーション（自治体名チェック等）や一意性制約（遺跡名＋ラベルの複合キー等）も併せて削除・緩和され、柔軟な運用が可能に。

---

## [0.2.12] - 2026-02-21

### 🏷️ 動的メタデータ（カスタムフィールド）機能
- **`ExtractedImage` への動的メタデータ追加機能の実装**
  - `extracted_images` テーブルに `custom_metadata` カラム（`jsonb`）を追加。
  - ラベリング画面 (`Step 4`) で、任意のキーと値のペアを無制限に追加・削除できる動的フォームを実装。
  - 入力されたメタデータは JSON 形式で保存され、空のキーや値は保存時に自動的にフィルタリング。
  - マイグレーション: `add_custom_metadata_to_extracted_images`
