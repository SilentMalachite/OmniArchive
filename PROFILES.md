...
OmniArchive は従来、`Archaeology` と `GeneralArchive` という 2 つの組み込みドメインプロファイルをコンパイル時に定義していました。バージョン 0.2.21 以降、YAML ファイルでドメインプロファイルを定義できるようになり、アプリケーション起動時に柔軟にプロファイルを切り替えることが可能になりました。
