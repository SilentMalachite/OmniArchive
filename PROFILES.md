...
フィールド単位の入力検証を定義します。キーは `metadata_fields` で定義済みのフィールド名（string key）で、値は検証ルール定義のマップです。外部入力由来のキーから atom は生成しません。
...
`field` は `metadata_fields` 内で定義済みのフィールドだけを参照できます。
...
`scope_field` と `label_field` は `metadata_fields` 内で定義済みのフィールドだけを参照できます。
...
フィールドキーは小文字・数字・アンダースコアのみの string として扱います。YAML プロファイルや DB カスタムフィールドから新しい atom を生成しないため、外部入力による atom table 枯渇を避けられます。
