...
- `lib/omni_archive_web/controllers/upload_asset_controller.ex:6` は `current_scope.user` と `Ingestion.get_pdf_source!(pdf_source_id, user)` により所有者/admin を確認してから `send_file/3` します。
...
対応状況:
- `lib/omni_archive_web/live/inspector_live/finalize.ex` の `mount/3` は、画像取得後に `Ingestion.get_pdf_source!(image.pdf_source_id, current_user)` を呼び、現在ユーザーがその `PdfSource` を取得できる場合だけ画面を表示します。
...
- 最終確認:
  - `mix precommit`: PASS (`430 tests, 0 failures, 4 skipped`)
- `mix review`: PASS
