defmodule OmniArchiveWeb.UserSessionControllerTest do
  ...
  describe "GET /users/log-in" do
    ...
    test "renders login page with email filled in (sudo mode)", %{conn: conn, user: user} do
      ...
      assert html =~ "セキュリティのため、再認証が必要です"
      refute html =~ "登録"
  ...
  describe "POST /users/log-in - email and password" do
    test "logs in successfully even if user results unconfirmed", %{conn: conn} do
    ...
    test "logs the user in with remember me", %{conn: conn, user: user} do
      ...
      assert conn.resp_cookies["_omni_archive_web_user_remember_me"]
      assert redirected_to(conn) == ~p"/"
    end
